$(function(){
	...
	$('form').submit(function(){
		...
		$(this).find('.check-error').first().find('input[data-validate],textarea[data-validate],select[data-validate]').first().focus().select();
		numError = 1;
		process($("form[name='"+this.name+"']")[0].name);
		return false;
	});
 ...
 * @returns
 */
function previewImage(file,dataId)  {  
  ...
  {  
    ...
    preview_img.onload = function(){  
      var rect = clacImgZoomParam(MAXWIDTH, MAXHEIGHT, preview_img.offsetWidth, preview_img.offsetHeight);  
      preview_img.width = rect.width;  
      preview_img.height = rect.height;  
      preview_img.style.marginLeft = rect.left+'px';  
      preview_img.style.marginTop = rect.top+'px';  
    };  
    var reader = new FileReader();  
    reader.onload = function(evt){
    	preview_img.src = evt.target.result;
  ...
  {  
    ...
    var preview_img = document.getElementById('preview_img');  
    preview_img.filters.item('DXImageTransform.Microsoft.AlphaImageLoader').src = src;  
    var rect = clacImgZoomParam(MAXWIDTH, MAXHEIGHT, preview_img.offsetWidth, preview_img.offsetHeight);  
    status =('rect:'+rect.top+','+rect.left+','+rect.width+','+rect.height);  
    preview_div.innerHTML = "<div id=divhead style='width:"+rect.width+"px;height:"+rect.height+"px;margin-top:"+rect.top+"px;margin-left:"+rect.left+"px;"+sFilter+src+"\"'></div>";  
  }  
}  
